...
#[no_mangle]
pub extern "C" fn macro_state_new() -> *mut MacroState {
    Box::into_raw(Box::new(MacroState { operators: HashMap::new(), brr: CString::new("").unwrap() }))
}

#[no_mangle]
pub extern "C" fn macro_state_free(ptr: *mut MacroState) {
    if ptr.is_null() {
        return;
    }
    unsafe {
...
#[no_mangle]
pub extern "C" fn macro_init_operator(
    runtime_state: *mut MacroState,
    operator_id: i32,
    parallel_worker_number: i32,
    plan_type: i32,
    optimizer_startup_cost: f64,
...
#[no_mangle]
pub extern "C" fn macro_operator_hash(
    query_template: *const c_char,
    macro_state: *mut MacroState,
) -> *const u8 {
    let state = unsafe { &mut *macro_state };
    let mut state_vec: Vec<(&(i32, i32), &OperatorState)> = state.operators.iter().collect();
    state_vec.sort_by(|a, b| { a.0.cmp(b.0) });

    let mut str = state_vec.iter().map(|(opid, op)| format!("{}-{},", opid.0, op.plan_type)).collect::<String>();
    let query_template = unsafe { CStr::from_ptr(query_template) }.to_str().unwrap();
    str.insert_str(0, query_template);

    state.brr = CString::new(str.trim_end_matches(",")).unwrap();
    unsafe { (*macro_state).brr.as_bytes_with_nul().as_ptr() }
}
...
#[no_mangle]
pub extern "C" fn cache_get_explain(redis_con: *mut RedisCon, intelligent: bool, query_text: *const c_char, plan_ms: f64) -> *mut CacheResult {
        ...
        Some(s) => {
            ...
            if let serde_json::Value::Array(ref mut veccy) = v {
                if let serde_json::Value::Object(ref mut map) = veccy[0] {
                    map.insert("Boot".into(), serde_json::Value::String("true".into()));
                    map.insert("Execution Time".into(), serde_json::Value::Number((now.elapsed().as_millis() as u64).into()));
                    map.insert("Planning Time".into(), serde_json::Value::from(plan_ms));
                }
            }
