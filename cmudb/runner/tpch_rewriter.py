...
class ExplainRewriter(Rewriter):
    def rewrite(self, query_num: int, query_subnum: int, query: str) -> (str, bool):
        ...
        if query[:10].lower().strip().startswith("select"):
            is_ea, query = True, f"EXPLAIN (FORMAT JSON, VERBOSE) {query}"
        # TODO(WAN): this is absolutely not an explain analyze. just need the same code path for a quick experiment.
        return query, is_ea
