...
def generate_metadata(
    source_file: Path, artifact_root, model_benchmark, model_sf, boot
):
    curpath = source_file
    metadata = {}
    ...
    if model_benchmark == "tpch":
        metadata["DefaultTimeout"] = Path(
            artifact_root
            / f"experiment/default/{str(model_benchmark)}/sf_{str(model_sf)}/{metadata['Seed']}/{metadata['Query']}.timeout"
        ).exists()
    elif model_benchmark == "dsb":
        metadata["DefaultTimeout"] = Path(
            artifact_root
            / f"experiment/default/{str(model_benchmark)}/sf_{str(model_sf)}/default/{metadata['Seed']}/{metadata['Query']}.timeout"
        ).exists()
    elif model_benchmark == "job":
        metadata["DefaultTimeout"] = Path(
            artifact_root
            / f"experiment/default/{str(model_benchmark)}/sf_{str(model_sf)}/{metadata['Seed']}/{metadata['Query']}.timeout"
        ).exists()
    else:
        raise Exception(f"Unknown: {model_benchmark}")
...
def shred(
    ...
    while len(shreddable) > 0:
        ...
        for k, v in generate_metadata(
            source_file, artifact_root, model_benchmark, model_sf, boot
        ).items():
            plan_cur[k] = v
        ...
        if "boot" in plan_cur["Experiment"]:
...
def load_results():
    ...
    if model_benchmark == "tpch":
        ...
        raise RuntimeError(f"Unknown: {str(model_benchmark)}")
    ...
    for planfile in planfiles:
        ...
        if len(contents) == 0:
            continue
        boot = contents.startswith(r"{'Boot': 'true',")
        j = eval(contents)
        operators.extend(
            shred(planfile, j, artifact_root, model_benchmark, model_sf, boot)
        )
    ...
    for toutfile in toutfiles:
        ...
        metadata = generate_metadata(
            toutfile, artifact_root, model_benchmark, model_sf, False
        )
...
def main():
    ...
    if model_benchmark == "tpch":
        ...
        # Follow TPC-H.
        rng = np.random.default_rng(15721)
        seeds = df.index.tolist()
        train_seeds = sorted(
            rng.choice(seeds, size=int(0.8 * len(seeds)), replace=False).tolist()
        )
        test_seeds = sorted(list(set(seeds) - set(train_seeds)))
        df.iloc[train_seeds, df.columns.get_loc("Seed")] = "seed_train"
        df.iloc[test_seeds, df.columns.get_loc("Seed")] = "seed_test"
        train_seeds = ["seed_train"]
        test_seeds = ["seed_test"]
    else:
        raise RuntimeError(f"Unknown: {model_benchmark}")
    ...
    test_df.to_parquet(model_root / "test_df.pq")
    ...
    for expt in df[(df["Benchmark"] == model_benchmark) & (df["SF"] == model_sf)][
        ...
        train_df.to_parquet(model_root / f"{expt}_train_df.pq")

        train_data = TabularDataset(train_df)
        test_data = TabularDataset(test_df)
        print(f"{LEGAL_COLS=} {target_col=}")
        print(f"{train_data.shape=} {test_data.shape=}")
        assert (train_data.shape[0] > 0) and (test_data.shape[0] > 0), "No rows?"
